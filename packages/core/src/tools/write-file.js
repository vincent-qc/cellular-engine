...
export class WriteFileTool extends BaseTool {
    ...
    async _getCorrectedFileContent(filePath, proposedContent, abortSignal) {
        ...
        if (fileExists) {
            ...
            const { params: correctedParams } = await ensureCorrectEdit(originalContent, {
                old_string: originalContent, // Treat entire current content as old_string
                new_string: proposedContent,
                file_path: filePath,
            }, this.client, abortSignal);
            correctedContent = correctedParams.new_string;
        }
        else {
            // This implies new file (ENOENT)
            correctedContent = await ensureCorrectFileContent(proposedContent, this.client, abortSignal);
        }
